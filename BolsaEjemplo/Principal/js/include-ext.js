...
(function() {
    ...
    if (includeCSS) {
        document.write('<link rel="stylesheet" type="text/css" href="' + path + '/Principal/css/ext-all' + suffix + '-debug.css"/>');
    }
    document.write('<script type="text/javascript" src="' + path + '/Principal/js/ext-all' + (rtl ? '-rtl' : '') + '.js"></script>');
    ...
    if (neptune) {
        ...
        neptunePath = (repoDevMode ? path + '/..' : path) +
            '/Principal/js/ext-theme-neptune' +
            (repoDevMode ? '-dev' : '') + '.js';
